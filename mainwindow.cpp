...
void MainWindow::displayImage(int, QImage image) {
    ...
    connect(this, SIGNAL(sendImage(QImage, int, int)), showImageWindow,
            SLOT(getImage(QImage, int, int)));
    if (ui->ImageRadio_Negative->isChecked()) {
        ...
        emit sendImage(image, 5, ui->value->value());
    } else {
        emit sendImage(image, 0, 0);
    }

    disconnect(this, SIGNAL(sendImage(QImage, int, int)), showImageWindow,
               SLOT(getImage(QImage, int, int)));
}
